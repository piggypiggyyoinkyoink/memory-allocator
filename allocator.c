...
void *mm_malloc(size_t size) {
    ...
    for (int i = 0; i < 3; i++){
        if (ptrArr[i] == NULL){
            for (int j = 0; j < i; j++){
                // free previously allocated nodes in this call
                mm_free((void *)((uint8_t*)ptrArr[j] + sizeof(struct Node)));
            }
    ...
    struct Node node1 = *ptrArr[0];
    struct Node node2 = *ptrArr[1];
    struct Node node3 = *ptrArr[2];
    node1.clone1 = ptrArr[1];
    node1.clone2 = ptrArr[2];
    node2.clone1 = ptrArr[0];
    node2.clone2 = ptrArr[2];
    node3.clone1 = ptrArr[0];
    node3.clone2 = ptrArr[1];
    *ptrArr[0] = node1;
    *ptrArr[1] = node2;
    *ptrArr[2] = node3;
